...
namespace H1_Mozarts_vals_generator
{
    internal class Program
    {
        ...
        #endregion
        #region Controller
        static void Controller(int times)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                sp.SoundLocation = GetFile(Convert.ToByte(i))[GetNumbers(Convert.ToByte(i))[dice1 + dice2, times]];
                sp.Load();
                sp.PlaySync();
            }
        ...
        #endregion
        #region Model
        static string[] GetFile(byte file)
        {
            if (file == 0)
            {
                string[] files = new string[175];
                for (int i = 1; i < 176; i++)
                {
                    files[i - 1] = @"C:\Users\zbc23jakl\source\repos\H1 Mozarts vals generator\H1 Mozarts vals generator\Wave\" + "M" + i + ".wav";
                }
                return files;
            }
            else
            {
                string[] files = new string[95];
                for (int i = 1; i < 96; i++)
                {
                    files[i - 1] = @"C:\Users\zbc23jakl\source\repos\H1 Mozarts vals generator\H1 Mozarts vals generator\Wave\" + "T" + i + ".wav";
                }
                return files;
            }

        }
        static int[,] GetNumbers(byte type)
        {
            if (type == 0)
            {
